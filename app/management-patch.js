...
(function(){
  ...
  function manualJobModal(){
    ...
    const opts=['<option value="">No customer selected</option>'].concat(customers.map(c=>`<option value="${c.id}">${window.esc?esc(c.full_name):c.full_name} · ${window.esc?esc(c.phone):c.phone}</option>`)).join('');
  ...
  async function saveManualJob(){
    ...
    try{
      const n=await window.SB.rpc('next_mtech_job_no');
      ...
      const row={
        ...
        status:document.getElementById('manualJobStatus').value,
        priority:document.getElementById('manualJobPriority').value,
        service_method:document.getElementById('manualJobMethod').value,
        diagnosis:document.getElementById('manualJobDiagnosis').value||null,
        scheduled_at:scheduled?new Date(scheduled).toISOString():null
      };
      const {error}=await window.SB.from('jobs').insert(row);
      ...
      if(btn){btn.disabled=false;btn.textContent='Create Manual Job';}
    }
  }
